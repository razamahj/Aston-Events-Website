...
<ul>
  ...
  <li><a class="active" href="/CRW/culture.php">Culture</a></li>
...
?>
	...
	<tr><th>Event Name</th> <th >Event Description</th> <th >Event Time</th> <th > </th><th ></tr>
<?php
		// Use select to get all student records and output in a table 
		$rows=$db->query("select event_name, event_description, event_time from event where category='culture' ");
		foreach ($rows as $row) { 
			echo "</td><td >" . $row['event_name']. "</td><td >" .$row['event_description']. "</td><td >" . $row['event_time']. "</td><td >" ."</td></tr>\n";
		}
		echo "</table> <br>";
