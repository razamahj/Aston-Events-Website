...
<ul>
  ...
  <li><a href="/CRW/other.php">Other</a></li>
   <li><a class="active" href="/CRW/EBD.php">Events By Date</a></li>
...
?>
	...
	<tr><th>Event Name</th> <th >Event Description</th> <th >Event Time</th> <th >Event Date</th> <th > </th><th ></tr>
<?php
		// Use select to get all student records and output in a table 
		$rows=$db->query("SELECT event_name, event_description, event_time, event_date FROM event WHERE event_date BETWEEN '2014-01-01' AND '2070-01-01' ");
		foreach ($rows as $row) { 
			echo "</td><td >" . $row['event_name']. "</td><td >" .$row['event_description']. "</td><td >" . $row['event_time']. "</td><td >" . $row['event_date']. "</td></tr>\n";
		}
		echo "</table> <br>";
